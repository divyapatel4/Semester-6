...
int main(int argc, char *argv[])
{
	...
	char *problem_name = "MATMUL_TRANSPOSE";
	...
	#pragma omp parallel for num_threads(P) private(i, j) shared(B, B_transpose) schedule(auto) collapse(2)
	for (i = 0; i < N; i++)
	{
		for (j = 0; j < N; j++)
		{
...
// Multiply A and B_transpose
	#pragma omp parallel for num_threads(P) private(i, j, k) shared(A, B_transpose, C) schedule(auto) 
	for (i = 0; i < N; i++)
	{
		for (j = 0; j < N; j++)
		{
			double sum = 0;
			#pragma omp simd reduction(+ : sum)		// SIMD directive is used to vectorize the loop and reduce the execution time and reduction is used to add the values of sum to avoid race condition
			for (k = 0; k < N; k++)
			{
