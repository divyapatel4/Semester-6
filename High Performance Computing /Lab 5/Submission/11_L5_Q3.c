        ...
        for (jj = 0; jj < n; jj += block_size)
        {
            for (kk = 0; kk < n; kk += block_size)
            {
                for (i = ii; i < min(ii + block_size,n); i++)
                {
                    ...
                    {
                        double sum = 0;
                        #pragma omp reduction(+:sum)		// Reduction clause is used to reduce the sum of all the values of sum in each thread to a single value and avoid race condition
...
int main(int argc, char *argv[])
{
	...
	char *problem_name = "MATMUL_BLOCK";
...
#pragma omp parallel for private(i, j) collapse(2)
	...
	int block_size = 32;			// block size is 32 as it is the best block size for this algorithm for L1 cache size of 32KB
