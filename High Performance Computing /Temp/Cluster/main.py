...
def compileFile(file):
    ...
    out, err = Popen('gcc-12 -o '+file_name+' '+file+' -fopenmp', stdout=PIPE, stderr=PIPE, shell=True).communicate()
    out = out.strip()
    err = err.strip()
...
if __name__ == "__main__":
    ...
    for p in range(procNumIndx, len(config.procs)):
    	for n in range(probSizeIndx,len(config.prob)):
        	...
        	#     f.write(dataPoint+'\n')
        	dataPoint = [prob_name,final_lscpu,config.prob[n],config.procs[p],(avg_data_algo[p][n]/runs),(avg_data_e2e[p][n]/runs)]
